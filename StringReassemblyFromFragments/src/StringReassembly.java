...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String combination = "";
        String strSub = str2.substring(overlap);
        combination = str1 + strSub;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        boolean checkSubsrting = true;
        Set<String> subString = strSet.newInstance();
        for (String set : strSet) {
            checkSubsrting = checkSubsrting & set.indexOf(str) < 0;
        }
        if (checkSubsrting) {
            for (String set : strSet) {
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        String printWithLineSeparators = text;
        for (int i = 0; i < printWithLineSeparators.length(); i++) {
            if (printWithLineSeparators.charAt(i) == '~') {
                String half = printWithLineSeparators.substring(0, i);
                String half2 = printWithLineSeparators.substring(i + 1,
                        text.length());
                printWithLineSeparators = half + "\n" + half2;
            }
        }
        out.println(printWithLineSeparators);
