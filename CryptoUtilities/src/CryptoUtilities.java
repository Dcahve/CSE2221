...
public final class CryptoUtilities {
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        NaturalNumber nCopyLocal = n.newInstance();
        nCopy.copyFrom(n);
        one.setFromInt(1);
        two.setFromInt(2);
        if (n.isZero() && p.isZero()) {
            nCopy.clear();
            nCopy.increment();
            NaturalNumber result = nCopy.divide(m);
            n.copyFrom(result);
            ...
            if (isEven(p) && !p.isZero()) {
                ...
                nCopyLocal.copyFrom(n);
                p.divide(two);
                powerMod(n, p, m);
                nCopy.copyFrom(n);
                n.multiply(nCopy);
                n.multiply(nCopyLocal);
                p.multiply(two);
                p.increment();
            }
            result = n.divide(m);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
         ...
         */
        return isWitnessToCompositness;
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        int i = 0;
        while (i < 20 && isPrime2 == true) {
            i++;
            NaturalNumber ncopy = n.newInstance();
            NaturalNumber oneCopy = n.newInstance();
            oneCopy.increment();
            ncopy.copyFrom(n);
            ncopy.decrement();
            NaturalNumber witness = randomNumber(n);
            while (!(witness.compareTo(oneCopy) > 0
                    && witness.compareTo(ncopy) < 0)) {
                witness = randomNumber(n);
            }
            isPrime2 = !isWitnessToCompositeness(witness, n);
